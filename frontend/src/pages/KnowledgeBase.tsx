        ...
        <header className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-[#6b5f57] dark:text-slate-400 text-sm lg:text-base max-w-2xl">
              Review and approve AI-generated knowledge articles
            </p>
            {autoApproveEnabled && (
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <label className="flex flex-col gap-2 text-sm font-medium text-[#6b5f57] dark:text-slate-300">
                Status
                <select
                  className="rounded-xl border border-[#F5ECE5] bg-white px-4 py-2.5 text-sm text-[#333333] transition-colors focus:outline-none focus:ring-2 focus:ring-[#E89F88]/40 dark:border-slate-600/50 dark:bg-slate-900/40 dark:text-white dark:focus:ring-blue-500/40"
                  value={statusFilter}
              ...
              <div className="flex flex-col gap-1 rounded-xl border border-[#F5ECE5] bg-[#FDF3EF] px-4 py-2.5 text-sm text-[#6b5f57] dark:border-slate-700/60 dark:bg-slate-900/40 dark:text-slate-300">
                <span className="text-xs uppercase tracking-wide text-[#E57252] dark:text-blue-300">Queue</span>
                <span className="text-xl font-semibold text-[#333333] dark:text-white">{visibleItems.length}</span>
              </div>
            </div>

            {error && (
              <div className="rounded-xl border border-red-200 bg-red-50/80 px-4 py-2.5 text-sm text-red-700 dark:border-red-800/60 dark:bg-red-500/10 dark:text-red-200">
                {error}
              </div>
            )}
          </div>
                  ...
                  {steps.length > 0 && (
                    <div className="space-y-2 rounded-xl border border-[#F5ECE5] bg-[#FDF3EF]/70 p-4 text-sm text-[#333333] dark:border-slate-700/60 dark:bg-slate-900/40 dark:text-slate-200">
                      ...
                      <ol className="space-y-1">
                        ...
                        {steps.length > 3 && (
                          <li className="text-xs text-[#6b5f57] dark:text-slate-400">+{steps.length - 3} more steps</li>
                        )}
                      </ol>
                    </div>
                  )}
